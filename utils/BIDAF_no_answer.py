...
from BIDAF_preprocess import gather_text, build_word_vocab, build_char_vocab, \
     save_noanswer_features, postprocess_df
...
def preprocess_noanswer_df(df):
    ...
    df['label'] = df.answers.apply(lambda an: [0 if len(an['text']) == 0 else an['answer_start'][0],
                                               0 if len(an['text']) == 0 else an['answer_start'][0] + len(an['text'][0])])
...
if __name__ == '__main__':
    ...
    word2idx = pickle.load(open('../data/no_answer/vocab_word2idx.pkl', 'rb'))
    idx2word = pickle.load(open('../data/no_answer/vocab_idx2word.pkl', 'rb'))
    char2idx = pickle.load(open('../data/no_answer/char2idx.pkl', 'rb'))
